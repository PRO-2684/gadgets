...
(function () {
    ...
    const configDesc = {
        ...
        hide: {
            ...
            items: {
                oneClick: {
                    name: "* 一键隐藏",
                    title: "为旧版代码块添加“隐藏代码”的按钮；一键隐藏所有置顶帖；添加隐藏回复的按钮",
                    type: "bool",
                    value: true,
                },
                warning: {
        ...
        display: {
            ...
            items: {
                ...
                cssFix: {
                    name: "CSS 修复",
                    title: "动态透明度；图标上光标不显示为 pointer",
                    type: "bool",
                    value: true,
                },
                imageMaxHeight: {
                    name: "限制图片最大高度",
                    title: "将帖子图片的最大高度限制为 70vh",
                    type: "bool",
                    value: false,
                },
                removeMinHeight: {
    ...
    const dynamicStyles = {
        "hide.warning":
            ".vw50_kfc_pb, .vw50_kfc_pt, .vw50_kfc_f { display: none; }",
        "hide.avatar": "td.pls > .pls.favatar .avatar { display: none; }",
        "hide.avatarDetail":
            "div.tns.xg2, dl.credit-list, p.md_ctrl, p.xg1, ul.xl.xl2.o.cl { display: none; }",
        "hide.rating": "div.pcb > h3.psth.xs1, dl.rate { display: none; }",
        "hide.comment": "div.pcb > div.cm { display: none; }",
        "hide.serial":
            "div.boxbg_7ree { background-image: none; padding-left: 0; }",
        "hide.background":
            "body, .psth, .pn { background-image: none; } textarea#fastpostmessage { background: none !important; }",
        "hide.top": "#toptb, #nv_ph, #nv, .comiis_nav { display: none; }",
        "hide.signature": "div.sign { display: none; }",
        "hide.allowTinySignature":
            "div.sign:not(:has(img)) { display: block; }",
        "display.regexFilter":
            "#threadlisttableid > .regex-filtered { display: none; }",
        ...
        "display.imageMaxHeight":
            "#postlist .plc .t_f img, #postlist .plc .tattl img { max-height: 70vh; }",
    ...
    function regexFilter(regexStr) {
        ...
        if (regexStr == "") {
            threads.forEach((thread) =>
                thread.classList.remove("regex-filtered"),
            );
            return;
        }
        ...
        if (threadsContainer) {
            const observer = new MutationObserver((mutations) => {
                const regex = new RegExp(
                    configProxy["display.regexFilter"],
                    "i",
                );
                mutations.forEach((mutation) => {
                    if (mutation.addedNodes.length) {
                        mutation.addedNodes.forEach((node) => {
                            if (
                                node.id &&
                                node.id.startsWith("normalthread_")
                            ) {
    ...
    function hideOneClick() {
        ...
        $$("em.viewsource").forEach((ele) => {
            const hide_code = ele.parentNode.appendChild(
        ...
        if (display && table) {
            ...
            const tooltip = $(
                "div#threadlist > div.th > table > tbody > tr > th > div.tf",
            );
            ...
            const hide_all = show_top.insertAdjacentElement(
                "beforeend",
        ...
        $$("table.plhin tbody:has( tr > td.pls)").forEach((ele) => {
            ...
            const header = ele.querySelector(
                "tr:nth-child(1) > td.pls > div.favatar div.authi",
            );
            header?.appendChild(toggle_reply_header);
            const blocked = ele.querySelector(
                "tr:nth-child(1) > td.plc > div.pct > div.pcb > div.locked",
            );
            if (blocked) {
    ...
    function getToTop() {
        ...
        if (nv)
            nv.addEventListener("dblclick", (e) => {
    ...
    function emojiFix() {
        ...
        function fixEmoji(html) {
            // Replace patterns like `&amp;#128077;` with represented emoji
        ...
        replies.forEach(fix);
        const signatures = $$(
            "div.sign, div.bm_c.u_profile > div:nth-child(1) > ul:nth-child(3) > li > table > tbody > tr > td",
        );
    ...
    function autoSign(enable) {
        if (enable && window === window.top) {
            ...
            if (
                ...
                !sign2 ||
                !url ||
                !url.startsWith("https://www.52pojie.cn/home.php?mod=task")
            )
                return;
            notify("normal", "后台签到中...");
            ...
            sign1.height = 20;
            sign1.src =
                "https://static.52pojie.cn/static/image/common/imageloading.gif";
            ...
            function check() {
                ...
                if (
                    curr ===
                        "https://www.52pojie.cn/home.php?mod=task&item=new" ||
                    curr.match(/^https:\/\/www\.52pojie\.cn\/*$/)
                ) {
                    notify("normal", "签到成功！");
                    iframe.remove();
                    msg = "签到成功！";
